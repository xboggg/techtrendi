-- TechTrendi Engagement Features Database Schema (FIXED)
...
-- Add XP and level to auth.users table
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='auth' AND table_name='users' AND column_name='raw_user_meta_data') THEN
    -- If auth.users doesn't have raw_user_meta_data, create a profiles table instead
...
-- Function to award XP and update level (using profiles table)
...
BEGIN
  -- Get user stats
  SELECT
    COALESCE(p.total_articles_read, 0) as total_articles_read,
    COALESCE(p.total_comments, 0) as total_comments,
    COALESCE(p.login_streak, 0) as login_streak,
...
DROP POLICY IF EXISTS "Users can add reactions" ON article_reactions;
DROP POLICY IF EXISTS "Users can remove their reactions" ON article_reactions;
...
-- Bookmarks policies
DROP POLICY IF EXISTS "Users can view their bookmarks" ON bookmarks;
DROP POLICY IF EXISTS "Users can create bookmarks" ON bookmarks;
DROP POLICY IF EXISTS "Users can update their bookmarks" ON bookmarks;
...
DROP POLICY IF EXISTS "Users can update reading progress" ON reading_history;
...
DROP POLICY IF EXISTS "Anyone can view share counts" ON article_shares;
DROP POLICY IF EXISTS "Users can log shares" ON article_shares;
...
COMMENT ON TABLE public.profiles IS 'User profiles with gamification stats (XP, level, streaks)';
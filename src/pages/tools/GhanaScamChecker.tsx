...
import { checkMessage, NUMBER_PATTERNS, type ScamIndicator } from "@/lib/scamPatterns";
...
export default function GhanaScamChecker() {
  ...
  let verdict: "high" | "medium" | "low" | "clear" | null = null;
  if (checked && input.trim()) {
    if (mode === "message") {
      ({ indicators, verdict } = checkMessage(input));
    } else {
      ...
      verdict = highCount >= 2 ? "high" : highCount === 1 || medCount >= 2 ? "medium" : indicators.length === 0 ? "clear" : "low";
    }
  }
